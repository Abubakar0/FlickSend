...
export default async function TransferDetailPage({
  ...
  const content = session.fixturePersonId ? (
    <TransfersBoundary currentPerson={developmentPersonById(session.fixturePersonId)} recordId={record} />
  ) : (
...
async function persistentTransferDetail(
  ...
  try {
    ...
    return <TransfersBoundary currentPerson={persistentPerson(account)} mode="persistent" recordId={recordId} />;
  } catch {
